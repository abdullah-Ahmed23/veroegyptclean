...
const ScrambleText = ({ text, className }: { text: string, className?: string }) => {
  ...
  const isInView = useTransform(useScroll({ target: containerRef, offset: ["start end", "end start"] }).scrollYProgress, [0, 1], [0, 1]); // Dummy transform to trigger hook
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 w-full group">
        <div className="relative p-6 border border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-foreground/50">
  ...
  const timeline = [
    {
      year: '2021',
      title: 'The Inception',
      description: 'Omar Nasseh and Mohamed El Deep met in Cairo. A fusion of design versus chaos. The vision: Anti-logo, Pro-quality.',
    },
    {
      year: '2022',
      title: 'Prototype Phase',
      description: '18 months of R&D. Fabric sourcing from the finest mills. Iteration #045 became the first Essential Hoodie.',
    },
    {
      year: '2023',
      title: 'Underground Growth',
      description: 'VERO spread via encrypted channels—word of mouth. A cult following emerged, valuing the unseen details.',
    },
    {
      year: '2024',
      title: 'System Expansion',
      description: 'Scaling the infrastructure. New categories loaded. Footwear and accessories protocols initiated.',
    },
  ];
    ...
    <div ref={containerRef} className="bg-background min-h-screen text-foreground overflow-x-hidden">
      ...
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-20 text-center px-4">
          ...
          >
            Decoding the matrix of modern luxury. <br />
            <span className="text-white font-medium">Built for the new world.</span>
          </motion.p>
        </div>
      </section>
          ...
          <div className="flex items-start gap-4 md:gap-8">
            ...
            <blockquote className="space-y-6">
              <p className="heading-3 md:heading-2 leading-tight">
                We founded VERO on a code deviation: <span className="text-muted-foreground line-through decoration-red-500/50">Loud Logos</span> vs <span className="bg-foreground text-background px-2">Quiet Power</span>.
                In a world of noise, silence is the ultimate flex.
              </p>
              <footer className="text-sm font-mono text-muted-foreground flex items-center gap-4">
                <span className="h-px w-12 bg-border" />
                <span>OMAR NASSEH & MOHAMED EL DEEP // FOUNDERS</span>
              </footer>
            </blockquote>
          </div>
        </div>
      ...
      <section className="section-padding">
        <div className="container-vero">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-4">
            <h2 className="heading-2 uppercase tracking-tighter"><ScrambleText text="CORE PROTOCOLS" /></h2>
            <span className="text-xs font-mono text-muted-foreground">[ LOADING VALUES... ]</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: "ZERO_COMPROMISE", desc: "Materials sourced from the void. Quality inspected at the molecular level." },
              { title: "ENGINEERED_FIT", desc: "Silhouettes designed for movement in the urban sprawl. Not just clothes, but armor." },
              { title: "SILENT_LUXURY", desc: "No signals needed. The quality creates its own frequency. Detected only by those who know." }
            ].map((val, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="group border border-border p-8 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors duration-300 relative overflow-hidden"
              >
                {/* Hover Glitch Line */}
                ...
                <h3 className="font-mono text-lg font-bold mb-4">{val.title}</h3>
                <p className="text-muted-foreground leading-relaxed">{val.desc}</p>

                <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity font-mono text-[10px] text-muted-foreground">
                  SEC_{i + 1}
                </div>
              </motion.div>
            ))}
      ...
      <section className="section-padding relative">
        ...
        <div className="container-vero relative z-10 space-y-2">
          <div className="text-center mb-16">
            <h2 className="heading-2"><ScrambleText text="TIMELINE LOG" /></h2>
          </div>
      ...
      <section className="h-[50vh] relative overflow-hidden group">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <h2 className="heading-1 text-white mix-blend-overlay opacity-50 select-none"><ScrambleText text="DESIGN LAB" /></h2>
        </div>
      </section>
