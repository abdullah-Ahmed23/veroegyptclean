-- 1. Modify order_items table to allow NULL product_id
-- This allows custom items that aren't linked to a pre-defined product
ALTER TABLE order_items ALTER COLUMN product_id DROP NOT NULL;

-- 2. Update the Secure Order Creation Function
-- We remove the requirement for a valid product_uuid for custom items
...
BEGIN
    ...
    LOOP
        ...
        -- Handle null product_id for custom items
        ...
        IF (item->>'variant_id') IS NULL OR (item->>'variant_id') = '' THEN
    ...
    FOR item IN SELECT * FROM jsonb_array_elements(items)
    LOOP
        -- Handle null variant_id or product_id
...
-- 3. Cleanup: Remove the placeholder product if it exists
-- This removes it from Shop and Products entirely
DELETE FROM products WHERE handle = 'custom-hoodie';
